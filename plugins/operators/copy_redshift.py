...
class CopyToRedshiftOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.log.info(
            "Coping data from {} to {} on table Redshift".format(s3_path, self.table)
        )

        formatted_sql = CopyToRedshiftOperator.copy_sql.format(self.table, s3_path)
        ...
        if self.file_format == "csv":
            ...
            formatted_sql += CopyToRedshiftOperator.parq.format(self.role_arn)

        self.log.info("Running Copy Command {}".format(formatted_sql))
        redshift.run(formatted_sql)
        self.log.info(
            "Successfully Copied data from {} to {} table on Redshift".format(
                s3_path, self.table
            )
        )
